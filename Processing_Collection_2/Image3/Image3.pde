...
void draw() {
   ...
   float BuildLength = 10000;                                                                           //The length of the building (set to a very high number so that all buildings reach the floor)
   ...
   float[] colorRGB = {40, 40, 40};

   //Generates each building from left to right, back to front
    for (int a=0; a<=3; a++){                                                                       //For each row a, higher value means more layers of buildings
      for (int i=0; i<=20; i++){                                                                        //For each building i,     
        ...
        BuildWidth = 150+BWidth+a*10;
        BuildHeight = 700+BHeight+a*a*5;
...
void genLightInput (float LeftX, float RightX, float Height, float absWidth) {
  float Window = absWidth/random(8);                                                                         //Determines a value for the length of a window based on the width of the building
    ...
    for (int h=0; h<=100; h++) {
      for (int a=0; a<=10; a++){
         ...
         rect(WindowX, Height+Window*2*h, Window, WindowY);
         }
      }
    }
}
